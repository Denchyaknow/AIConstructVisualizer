...
export function createShardmind(PIXI) {
  ...
  return {
    ...
    render(ctx) {
      ...
      const spreadX = 1 + (spread - 1) * .28;
      const spreadY = 1 + (spread - 1) * .20;
      const fractureSpread = .45 + spread * .55;
      const facetBowSpread = .35 + spread * .65;
      const stretchX = lerp(.72, 1.08, profile.curiosity) * (1 + Math.sin(time * .19) * .035) * spreadX;
      const stretchY = lerp(1.04, .82, profile.coherence) * (1 + Math.cos(time * .23) * .03) * spreadY;
      const tilt = -.13 + pointer.x * .09 + Math.sin(time * .08) * .05;
      const cos = Math.cos(tilt);
      const sin = Math.sin(tilt);
      const points = BASE_POINTS.map((point, index) => {
        const fracture = (profile.creativity * .09 + effect('correction') * .08) * Math.sin(time * .72 + index * 2.17) * fractureSpread;
      ...
      const paths = CONNECTIONS.map(([from, to], index) => {
        ...
        return {
          ...
          color,
          sample(t) {
            const bow = Math.sin(Math.PI * t) * layout.scale * .018 * (index % 2 ? 1 : -1) * profile.creativity * facetBowSpread;
      ...
      return {
        ...
        debug: {
          spread,
          maxPointDistance: Number(Math.max(...points.map(point => Math.hypot(point.x - layout.cx, point.y - layout.cy))).toFixed(2)),
          fractureSpread: Number(fractureSpread.toFixed(2)),
          facetBowSpread: Number(facetBowSpread.toFixed(2)),
        },
      };
    },
  };
