...
function branchGeometry(branch, index, ctx) {
  const { layout, time, profile, tuning } = ctx;
  const spread = tuning?.spread ?? 1;
  const reach = 1 + (spread - 1) * .32;
  const bendScale = .20 + spread * .80;
  const breathe = 1 + Math.sin(time * .31 + branch.seed) * (.025 + profile.creativity * .035);
  const length = branch.length * breathe * reach;
  const start = { x: layout.cx, y: layout.cy };
  const p1n = polar(length * .24, branch.angle + branch.bend * .75 * bendScale);
  const p2n = polar(length * .60, branch.angle - branch.bend * .35 * bendScale + Math.sin(time * .19 + index) * .035 * profile.curiosity * (.40 + spread * .60));
  const endn = polar(length, branch.angle + branch.bend * .16 * bendScale);
  return {
    start,
...
export function createHyphae(PIXI) {
  ...
  return {
    container,
    render(ctx) {
      const { layout, time, profile, effect, effectBoost, tuning } = ctx;
      const spread = tuning?.spread ?? 1;
      const secondaryDivergence = .25 + spread * .75;
      const secondaryReach = 1 + (spread - 1) * .45;
      const geometries = BRANCHES.map((branch, index) => branchGeometry(branch, index, ctx));
      bed.clear();
      vessels.clear();
      highlights.clear();
      spores.clear();
      bed.circle(layout.cx, layout.cy, layout.scale * (.80 + profile.curiosity * .04) * (1 + (spread - 1) * .18))
      ...
      geometries.forEach((geometry, index) => {
        ...
        for (let side = -1; side <= 1; side += 2) {
          ...
          const endAngle = BRANCHES[index].angle + side * lerp(.24, .52, profile.curiosity) * secondaryDivergence;
          const length = layout.scale * (.20 + (index % 3) * .025) * secondaryReach;
          const control = {
            x: origin.x + Math.cos(endAngle - side * .18 * (.30 + spread * .70)) * length * .52,
            y: origin.y + Math.sin(endAngle - side * .18 * (.30 + spread * .70)) * length * .52,
          };
      ...
      for (const spore of sporeSeeds) {
        ...
        const offset = spore.offset * layout.scale * (1 + profile.curiosity * .4) * (.50 + spread * .50);
      ...
      return {
        ...
        debug: {
          spread,
          maxNodeDistance: Number(Math.max(...geometries.map(geometry => Math.hypot(geometry.end.x - layout.cx, geometry.end.y - layout.cy))).toFixed(2)),
          mainBendMultiplier: Number(geometries[0].bendScale.toFixed(2)),
          secondaryDivergence: Number(secondaryDivergence.toFixed(2)),
          secondaryReach: Number(secondaryReach.toFixed(2)),
        },
      };
    },
  };
